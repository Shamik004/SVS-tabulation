...
if (fileInput && sheetSelect && downloadBtn && clearBtn && statusText && summaryPanel && excelTable) {
      ...
      try {
        ...
        if (!Object.keys(result.outputSheets).length) {
          ...
          uploadHistory.unshift({
            fileName: file.name,
            status: "Skipped",
            detail: "No usable 1st-year sheets found",
          });
          continue;
        }
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    XLSX.writeFile(outputBook, "combined_first_year_data.xlsx");
  });
  ...
  function processWorkbook(workbook) {
    ...
    workbook.SheetNames.forEach((sheetName) => {
      const identity = parseFirstYearSheetIdentity(sheetName);
      ...
      const parsed = parseTabulationSheet(rows, identity);

      if (!parsed.students.length) {
        return;
      }

      const built = buildFirstYearSheet(parsed);
      outputSheets[identity.outputName] = built.rows;
    });

    return { outputSheets };
  }

  function parseFirstYearSheetIdentity(sheetName) {
    const upper = sheetName.toUpperCase();

    if (upper.includes("2ND") || upper.includes("SECOND") || upper.includes("LATERAL")) {
      return null;
    }
    ...
    const hasFirstMarker = upper.includes("1ST") || upper.includes("FIRST");

    return {
      code,
      outputName: hasFirstMarker ? `${code} 1ST YEAR` : code,
    };
  }
  ...
  function parseTabulationSheet(rows, identity) {
    ...
    if (headerRowIndex < 0) {
      return { code: identity.code, paperLabels: [], students: [], hasGrandTotal: false };
    }
    ...
    return {
      ...
      paperLabels,
      students,
      hasGrandTotal: extraColumns.grandTotal >= 0,
    };
  }
  ...
  function getExtraColumnIndexes(headerRow) {
    return {
      ...
      mSlNo: findColumnIndex(headerRow, ["M", "SL", "NO"], ["SERIAL", "NO"]),
  ...
  function buildFirstYearSheet(parsed) {
    const headers = ["NAME", "ENROLLMENT NO", "SC NAME", "ROLL", "ER", "M. SL. NO.", "GRADE", "RESULT"];

    if (parsed.hasGrandTotal) {
      headers.push("GRAND TOTAL");
    }

    parsed.paperLabels.forEach((label) => {
      headers.push(`${label} ASSIGNMENT (F.M 100)`);
      headers.push(`${label} ASSIGNMENT`);
      headers.push(`${label} T.E.E (F.M 100)`);
      headers.push(`${label} T.E.E 70`);
      headers.push(`${label} TOTAL F.M 100`);
      headers.push(`${label} C/NC`);
    });
    ...
    parsed.students.forEach((student) => {
      ...
      if (parsed.hasGrandTotal) {
        row.push(student.extra.grandTotal);
      }

      parsed.paperLabels.forEach((label) => {
        const marks = student.papers[label];
        row.push(marks ? marks.assignmentFm100 : "");
        row.push(marks ? marks.assignment : "");
        row.push(marks ? marks.teeFm100 : "");
        row.push(marks ? marks.tee70 : "");
        row.push(marks ? marks.total : "");
        row.push(marks ? marks.cnc : "");
      });
